...
export interface MotionVerdict {
  ...
  /**
   * De clip beweegt wel netjes, maar het heeft niets met de gesproken zin te
   * maken. Geen afkeuring op zichzelf — een rustige kloppende clip mag blijven —
   * maar bij een herkansing weegt het wel mee, en het verlaagt de score zodat een
   * latere poging mét verband wint.
   */
  losseBeweging: boolean;
}
...
export async function critiqueMotion(opts: {
  ...
  if (opts.frames.length === 0) {
    return { ok: true, score: 5, reason: "niet beoordeeld (geen frames)", betterSteer: null, addedElements: false, losseBeweging: false };
  }
...
${opts.voiceover
  ? `KIJK OOK NAAR HET VERBAND MET DE ZIN. De beweging hoort te passen bij wat er in deze scène gezegd wordt. Beweegt er iets willekeurigs dat niets met de zin te maken heeft — iemand knippert met de ogen terwijl de zin over een voorwerp gaat — zet dan "losseBeweging" op true en schrijf in "betterSteer" welke beweging er wél bij de zin hoort. Dat is op zichzelf geen afkeuring: een rustige, kloppende clip zonder verband mag blijven staan als er verder niets mis is, maar bij een herkansing weten we dan wat er beter kan.`
  : ""}
...
Antwoord met JSON: {"ok": boolean, "score": <0-10>, "addedElements": boolean, "losseBeweging": boolean, "reason": "<korte reden in het Nederlands>", "betterSteer": "<kortere, nóg voorzichtiger NL-bijsturing die dichter bij de bepaalde beweging blijft, of null als ok=true>"}.`;
  ...
  try {
    ...
    const parsed = JSON.parse(res.choices[0]?.message?.content ?? "{}") as { ok?: boolean; score?: number; addedElements?: boolean; losseBeweging?: boolean; reason?: string; betterSteer?: string | null };
    const addedElements = parsed.addedElements === true;
    const losseBeweging = parsed.losseBeweging === true;
    ...
    const ruweScore = typeof parsed.score === "number" ? Math.max(0, Math.min(10, parsed.score)) : (ok ? 8 : 3);
    // Beweging zonder verband met de zin telt als halve winst: netjes, maar niet
    // waar het om ging. Zo wint een latere poging die wél bij de zin past.
    const score = losseBeweging ? Math.max(0, ruweScore - 3) : ruweScore;
    return { ok, score, reason: String(parsed.reason ?? ""), betterSteer: parsed.betterSteer ?? null, addedElements, losseBeweging };
  } catch (e) {
    ...
    return { ok: true, score: 5, reason: "beoordeling niet gelukt", betterSteer: null, addedElements: false, losseBeweging: false };
  }
}
