...
// De overlay is pure SVG (zie StoryScene) en bestaat sinds het vervallen van de
// koppen en grote getallen alleen nog uit het merklogo. We rasteren die
// server-side met resvg i.p.v. een headless browser: betrouwbaar op Vercel (geen
// chromium) en snel.
//
// De overheidsmodus gaat een stap verder: die scenes zijn helemaal SVG en worden
// hier frame voor frame gerasterd. Daar staat wél echte tekst in, dus de fonts
// moeten geladen worden — anders exporteert de video zonder letters.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const media: { file: string; isVideo: boolean; clipDur: number; isLayout?: boolean }[] = [];
    for (let i = 0; i < N; i++) {
      const s = spec.scenes[i];
      // Overheidsmodus: deze scene wordt straks zelf getekend, frame voor frame.
      if (s.layout) { media.push({ file: "", isVideo: false, clipDur: 0, isLayout: true }); continue; }
      if (s.videoUrl) {
    ...
    // ── 2. Transparante logo-overlay (resvg, geen browser) ───────────
    ...
    // segment gebakken (als er een logo is). De voice-over loopt er als doorlopend
    // spoor onder, dus de harde cut valt visueel nauwelijks op.
    const segPaths: string[] = [];
    // Palet van de zelfgetekende scenes: dezelfde huisstijlkleuren als de preview.
    const overheidKleuren = kleurenUitHuisstijl(spec.navy, spec.accent);

    for (let i = 0; i < N; i++) {
      const segDur = windows[i].duration;
      const segPathVoor = path.join(dir, `seg-${String(i).padStart(3, "0")}.mp4`);

      // ── Zelfgetekende scene: frame voor frame rasteren met resvg ──────
      // Geen beeldmodel en geen browser. Dezelfde bouwfunctie als de preview,
      // dus wat de gebruiker in de editor ziet is exact wat hier uit komt.
      if (media[i].isLayout) {
        const scene = spec.scenes[i];
        const fdir = path.join(dir, `sc-${String(i).padStart(3, "0")}`);
        await mkdir(fdir, { recursive: true });
        const frames = Math.max(1, Math.round(segDur * STORY_FPS));
        for (let f = 0; f < frames; f++) {
          const svgFrame = bouwOverheidSvg(scene.layout!, {
            format: spec.format,
            t: f / STORY_FPS,
            duur: segDur,
          ...
          const framePng = new Resvg(svgFrame, {
            fitTo: { mode: "width", value: W },
            font: { fontFiles: FONT_FILES, defaultFontFamily: "Inter", loadSystemFonts: false },
          }).render().asPng();
          await writeFile(path.join(fdir, `f-${String(f).padStart(5, "0")}.png`), framePng);
        }
        await runFfmpeg([
          "-framerate", String(STORY_FPS), "-i", path.join(fdir, "f-%05d.png"),
          "-c:v", "libx264", "-preset", "veryfast", "-pix_fmt", "yuv420p",
          "-r", String(STORY_FPS), "-video_track_timescale", String(STORY_FPS * 1000),
          "-t", segDur.toFixed(3), "-y", segPathVoor,
        ]);
        segPaths.push(segPathVoor);
        continue;
      }
      ...
      await runFfmpeg([
        "-i", media[i].file, ...(overlayPath ? ["-i", overlayPath] : []),
